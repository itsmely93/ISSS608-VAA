...
5.  `ggraph`: for creation of network graphs.

6.  `visNetwork`: for creating interactive network graphs.

7.  `plotly`: for creating interactive charts.

8.  `tidytext`: for text mining.

9.  `widyr`: for counting and correlating word pairs.
...
```{r}
pacman::p_load(jsonlite, tidygraph, ggraph, visNetwork, skimr, tidytext, tidyverse, knitr, plotly, widyr, igraph)
```
