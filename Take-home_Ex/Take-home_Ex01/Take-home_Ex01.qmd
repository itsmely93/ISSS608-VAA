...
3.  `knitr`: for generating a simple table using the `kable()` function

4.  `ggstatsplot`: for creating graphics that include details from statistical tests

5.  `ggridges`: for creating ridgeline plots

6.  `ggmosaic`: for creating mosaic plots
...
```{r}
pacman::p_load(plotly, tidyverse, knitr, ggstatsplot, ggridges, ggmosaic)
```
...
1.  Most participants in are highly educated, with only 8.31% of the participants surveyed having a "Low" level of education. More than half of the participants (51.9%) have an education level of "HighSchoolOrCollege".

2.  Household sizes of the participants are relatively small, with more than 2/3 of the participants' (71.2%) household sizes being 2 or less.
...
### 4.1.6 Average spending, earning and savings by household size

```{r}
fin_mean <- fin_journal %>% 
  group_by(participantId) %>%
  summarise(mean_expenses = mean(total_expenses), mean_earnings = mean(total_earnings), mean_savings = mean(total_savings))

joined <- merge(participants, fin_mean, by = "participantId")
...
fig2 <- plot_ly(
  ...
  showlegend = FALSE,
  notched = TRUE) %>%
  layout(title = "Average monthly spending by household size?",
         xaxis = list(title = list(text = "Household Size")),
         yaxis = list(title = list(text = "Average Amount Spent ($)")))
...
fig3 <- plot_ly(
  ...
  showlegend = FALSE,
  notched = TRUE) %>%
  layout(title = "Average monthly savings by household size?",
         xaxis = list(title = list(text = "Household Size")),
         yaxis = list(title = list(text = "Average Amount Saved ($)")))
...
### 4.2.2 Does having kids affect the average monthly spending, savings and/on earnings?
...
kids_spend <- ggplot(data = joined, aes(x = educationLevel, y = mean_expenses, fill = educationLevel)) +
  ...
  xlab("Education Level") + ylab("Average Monthly Spending") +
  theme(legend.position = "none", 
...
ggplotly(kids_spend)
```
The presence of children in the household generally leads to increased average monthly spending across all educational levels. Nevertheless, participants with a "Low" education level spend less on average per month when they have kids, compared to those without kids. This finding, however, may be attributed to the influence of outliers.
...
In summary, participants who have kids earn, spend and save more than partipants who do not have kids.

### 4.2.2 Is there a correlation between age and average monthly spending, savings and/or earnings?
...
#| code-fold: true
#| eval: false
spend_age <- ggscatterstats(
  data = joined,
  x = age,
  y = mean_expenses,
  xlab = "Age",
  ylab = "Average Monthly Spending ($)",
  title = "Is age correlated with average monthly spending?")
...
Based on the three scatter plots generated, there does not appear to be a correlation between age and monthly average spending, saving and earning.

### 4.2.3 Does education level affect average monthly spending, savings and/or earnings?

To determine if the level of education affects the participants' average monthly spending/savings/earnings, a oneway ANOVA test is conducted on the following hypotheses at a 95% confidence level.
...
edu_spend <- ggbetweenstats(
  data = joined,
  x = educationLevel,
  y = mean_expenses,
  xlab = "Education Level",
  ylab = "Average Monthly Spending ($)",
  title = "Does education level affect average monthly spending?")
...
As the p-value of 5.10e-23 is smaller than the alpha value of 0.05, we can reject the null hypothesis as there is statistical evidence to conclude that there is a difference among the means of the average monthly spending of the different education levels.

The chart illustrates that as the education level rises, the average monthly expenditure increases as well. Participants who have "low" levels of education tend to spend the least, with an average monthly spending of \$887.47. On the other hand, those with the highest education level ("Graduate"), spend the most each month, spending an average of \$1462.08.
...
The three charts above demonstrate that participants with "Low" level of education have the lowest average monthly spending, earnings, and savings compared to other education levels.

This observation is to be expected as their average monthly earnings are also comparatively lower, which will thus result in lower spending and savings amount. Thus, evaluating their spending and saving patterns based on the average monthly earnings alone may not provide a complete picture. It would be more appropriate to analyze their spending and saving habits in relation to the proportion of their earnings.

### 4.2.4 Comparing spending/savings in proportion of earnings
...
plot_ly(data=total_mean,
    ...
    add_trace(y = ~spending_prop,
              name = 'Spending',
              hoverinfo = "text",
              text = ~paste("Spending", spending_prop,"%"),
              textposition = 'none') %>%
